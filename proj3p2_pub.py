...
# [Total cost, cost to go (not based on goal location), index, parent, [x, y, theta], distance traveled to reach the point, distance in x y and theta to reach point]
        ...
        for i in range(0,8):
            ...
            if check_ob == 1: #decoupled the checks into separate statements
                ...
                if check_ol == 0:
                    ...
                    new_node = [lxu, cost_come, id, cur_ind, new_pos, step_L, move_dist]
                    ...
                    if m_co >  lxu:
                        for j in range(0, open_l.qsize()):
                            ...
                            if i_c == (round(new_pos[0]/10+50)) and j_c == (round(new_pos[1]/10+100)):
                                ...
                                imp_q = [lxu, cost_come, rep_ind, cur_ind, rep_pos, step_L, move_dist]
                                open_l.put(imp_q)
                                break
            elif check_cl == 1:
                continue
...
for i_pa in range(0, len_pa):
    ...
    for j_pa in range(0, plt1_size):
        if closed_l.queue[j_pa][2] == ind_pa:
            ...
            dist_pa.append(closed_l.queue[j_pa][6])

# define the publisher node function
def astar_path():
    ...
    for move in dist_pa:
        speed = (np.sqrt(move[0]**2 + move[1]**2))/dt
        turn = move[2]
